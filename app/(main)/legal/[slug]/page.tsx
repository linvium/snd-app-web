...
/**
 * One route for every legal document: /legal/terms, /legal/privacy, and
 * whatever the next one is called. Adding a page is an insert, not a file.
 */
export const revalidate = 300

export async function generateStaticParams() {
  const pages = await loadAllPageSlugs(createPublicClient())
  return pages.filter((page) => page.category === 'legal').map(({ slug }) => ({ slug }))
}

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { slug } = await params
  return buildPageMetadata('legal', slug)
}
...
export default async function LegalPage({ params }: PageProps) {
  ...
  if (page.category !== 'legal') redirect(pagePath(page.category, page.slug))
